...
with open("stopwords.txt", "r") as f:
    stopword = [line.strip() for line in f.readlines()]  # Remove whitespace/newlines
...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    hlf = max(0,1-(label*(np.dot(feature_vector,theta)+theta_0)))
    return hlf
...
def perceptron_single_step_update(
    ...
    """
    fx = label * (np.dot(feature_vector,current_theta)+current_theta_0)
    if fx<=0:
...
def perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):  # Loop through dataset T times
        for i in get_order(feature_matrix.shape[0]):  # Iterate through each sample
            current_theta,current_theta_0 = perceptron_single_step_update(feature_matrix[i],labels[i],current_theta,current_theta_0)
            #print(t,current_theta[0],current_theta[1],current_theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    sum_theta = np.zeros(feature_matrix.shape[1], dtype=np.float64)
    sum_theta_0 = np.float64(0)

    n = feature_matrix.shape[0]
    for t in range(T):  # Loop through dataset T times
        for i in get_order(n):  # Iterate through each sample
            current_theta,current_theta_0 = perceptron_single_step_update(feature_matrix[i],labels[i],current_theta,current_theta_0)
            sum_theta += current_theta
            sum_theta_0 += current_theta_0

            #print(t,sum_theta / ((i+1)*(t+1)), sum_theta_0 / ((i+1)*(t+1)))
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):  # Loop through dataset T times
        for i in get_order(n):  # Iterate through each sample
            ...
            #Sprint(t,theta[0],theta[1],theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    y_hat = np.dot(feature_matrix,theta) + theta_0
...
def classifier_accuracy(
    ...
    training_accuracy = accuracy(train_preds,train_labels)
    validation_accuracy = accuracy(val_preds,val_labels)
