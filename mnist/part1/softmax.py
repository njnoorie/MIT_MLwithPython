...
def compute_probabilities(X, theta, temp_parameter):
    ...
    for i in range(n):
        for j in range(k):
            H[j][i] = np.exp((np.dot(theta[j], X[i]) / temp_parameter) - c[i])
        H[:, i] /= np.sum(H[:, i])
    return H
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    for i in range(n):
        c += -np.log(H[Y[i]][i])
    c /= n
    c += (lambda_factor / 2) * np.sum(theta ** 2)
    return c
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    gradient = -np.dot(indicator - H, X) / (temp_parameter * n) + lambda_factor * theta
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    Y_mod_3, Y_pred_mod_3 =  Y % 3, Y_pred % 3
    return 1 - np.mean(Y_mod_3 == Y_pred_mod_3)
